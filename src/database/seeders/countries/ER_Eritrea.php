<?php

namespace Lwwcas\LaravelCountries\Database\Seeders\Countries;

use Lwwcas\LaravelCountries\Database\Seeders\Builder;
use Lwwcas\LaravelRssReader\Abstract\CountrySeeder;

class ER_Eritrea extends CountrySeeder
{

    /**
     * Attribute that defines the language of countries
     *
     * @var string
     */
    public $lang = 'en';

    /**
     * Attribute that defines the language of countries
     *
     * @var string
     */
    public $region = 'africa';

    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        // Set the country's common name
        $this->name = 'Eritrea';

        // Set the country's official name
        $this->official_name = 'State of Eritrea';

        // Set the ISO 3166-1 alpha-2 code
        $this->iso_alpha_2 = 'ER';

        // Set the ISO 3166-1 alpha-3 code
        $this->iso_alpha_3 = 'ERI';

        // Set the ISO 3166-1 numeric code (usually three digits)
        $this->iso_numeric = '232';

        // Set the international dialing code
        $this->international_phone = '291';

        // Define the languages spoken in the country
        $this->languages = ['ti', 'ar', 'en']; // Tigrinya, Arabic, and English are the main languages

        // Define the top-level domain(s)
        $this->tld = ['.er'];

        // Define alternative top-level domains
        $this->alternative_tlds = []; // No significant alternative TLDs

        // Define average internet speeds
        $this->internet_speed = [
            // Average fixed broadband speed
            'average_speed_fixed' => '2 Mbps',
            // Average mobile connection speed
            'average_speed_mobile' => '1 Mbps',
        ];

        // Define internet penetration rate
        $this->internet_penetration = '1%'; // One of the lowest internet penetration rates in the world due to limited infrastructure

        // Set the national cybersecurity agency
        $this->cybersecurity_agency = null; // Eritrea lacks a formal cybersecurity agency

        // List popular web technologies in Eritrea
        $this->popular_technologies = ['PHP', 'HTML', 'CSS'];

        // Set the WMO (World Meteorological Organization) country code
        $this->wmo = 'ER';

        // Set the GeoName ID for geographical data reference
        $this->geoname_id = '338010';

        // Define the currency information
        $this->currency = [
            // Currency name
            'name' => 'Eritrean nakfa',
            // Currency code
            'code' => 'ERN',
            // Currency symbol
            'symbol' => 'Nfk',
            // Main unit
            'main_unit' => 'nakfa',
            // Subunit
            'sub_unit' => 'cent',
            // Subunit to main unit conversion
            'sub_unit_to_unit' => 100,
            // Banknotes in circulation
            'banknotes' => ['1', '5', '10', '20', '50', '100'],
            // Main coins
            'coins_main' => ['1'],
            // Subunit coins
            'coins_sub' => ['1', '5', '10', '25', '50 cents'],
        ];

        // Set the population
        $this->population = 3546000; // As per recent estimates

        // Set the area in square kilometers
        $this->area = 117600; // km²

        // Set the capital city
        $this->capital = 'Asmara';

        // Define the time zone(s) using IANA time zone identifiers
        $this->timezones = [
            [
                // Time zone identifier
                'timezone_id' => 'Africa/Asmara',
                // Standard time offset
                'standard_time' => 'UTC+3', // East Africa Time (EAT)
                // No daylight saving time observed
            ],
        ];

        // Set the independence day
        $this->independence_day = '1993-05-24'; // Date of independence from Ethiopia

        // List international organizations the country is part of
        $this->international_organizations = [
            'United Nations',
            'African Union',
            'World Trade Organization (WTO)',
            'International Monetary Fund (IMF)',
            'World Health Organization (WHO)',
            'Intergovernmental Authority on Development (IGAD)',
        ];

        // Set the GDP in billions of USD
        $this->gdp = 6.7; // GDP in billions of USD

        // List the predominant religions
        $this->religions = [
            'Christianity (Eastern Orthodox)',
            'Islam (Sunni)',
        ];

        // Describe the form of government
        $this->government = 'Unitary one-party presidential republic';

        // Set the national sport
        $this->national_sport = 'Cycling'; // Cycling is a very popular sport in Eritrea

        // Define bordering countries
        $this->borders = [
            ['name' => 'Sudan', 'iso_alpha_2' => 'SD'],
            ['name' => 'Ethiopia', 'iso_alpha_2' => 'ET'],
            ['name' => 'Djibouti', 'iso_alpha_2' => 'DJ'],
        ];

        // Define the country's flag emoji representation
        $this->emoji = [
            // Native emoji image
            'img' => '🇪🇷',
            // Unicode code points
            'uCode' => 'U+1F1EA U+1F1F7',
            // HTML entities
            'html' => '&#x1F1EA;&#x1F1F7;',
            // CSS entities
            'css' => '\\1F1EA\\1F1F7',
            // Decimal Unicode entities
            'decimal' => '&#127466;&#127479;',
            // Direct UTF-8 string
            'utf8' => '🇪🇷',
            // UTF-16 surrogate pairs
            'utf16' => '\uD83C\uDDEA\uD83C\uDDF7',
            // Shortcode in Emojione, Slack
            'shortcode' => ':flag-er:',
            // Hexadecimal entity
            'hex' => '&#x1F1EA;&#x1F1F7;',
        ];

        // Define the colors of the country's flag with accurate color values
        $this->flag_colors = [
            [
                // Color name
                'name' => 'Red',
                // Web color name
                'web_name' => 'red',
                // Hexadecimal color code
                'hex' => '#C8102E',
                // RGB color values
                'rgb' => '200,16,46',
                // CMYK color values
                'cmyk' => '0,92,77,22',
                // HSL color values
                'hsl' => '350,86%,42%',
                // HSV color values
                'hsv' => '350,92%,78%',
                // Pantone color code
                'pantone' => '186 C',
                // Contrast color for readability
                'contrast' => '#FFFFFF',
            ],
            [
                // Color name
                'name' => 'Green',
                // Web color name
                'web_name' => 'green',
                // Hexadecimal color code
                'hex' => '#007A33',
                // RGB color values
                'rgb' => '0,122,51',
                // CMYK color values
                'cmyk' => '100,0,58,52',
                // HSL color values
                'hsl' => '145,100%,24%',
                // HSV color values
                'hsv' => '145,100%,48%',
                // Pantone color code
                'pantone' => '349 C',
                // Contrast color for readability
                'contrast' => '#FFFFFF',
            ],
            [
                // Color name
                'name' => 'Blue',
                // Web color name
                'web_name' => 'blue',
                // Hexadecimal color code
                'hex' => '#0033A0',
                // RGB color values
                'rgb' => '0,51,160',
                // CMYK color values
                'cmyk' => '100,69,0,37',
                // HSL color values
                'hsl' => '227,100%,31%',
                // HSV color values
                'hsv' => '227,100%,63%',
                // Pantone color code
                'pantone' => 'Reflex Blue C',
                // Contrast color for readability
                'contrast' => '#FFFFFF',
            ],
            [
                // Color name
                'name' => 'Gold (Olive Branch Emblem)',
                // Web color name
                'web_name' => 'gold',
                // Hexadecimal color code
                'hex' => '#FFD700',
                // RGB color values
                'rgb' => '255,215,0',
                // CMYK color values
                'cmyk' => '0,16,100,0',
                // HSL color values
                'hsl' => '51,100%,50%',
                // HSV color values
                'hsv' => '51,100%,100%',
                // Pantone color code
                'pantone' => '116 C',
                // Contrast color for readability
                'contrast' => '#000000',
            ],
        ];

        // Define the geographical coordinates
        $this->coordinates = [
            // Latitude in decimal degrees
            'latitude' => '15.1794',
            // Longitude in decimal degrees
            'longitude' => '39.7823',
            // Degrees with decimal
            'dd' => '15.1794° N, 39.7823° E',
            // Degrees, minutes, seconds
            'dms' => '15°10\'45.84" N, 39°46\'56.28" E',
            // Degrees and decimal minutes
            'dm' => '15°10.764\' N, 39°46.938\' E',
            // Various GPS formats
            'gps' => [
                [
                    // Format description
                    'format' => '[N,S]dd°mm\'ss.ssss", [E,W]ddd°mm\'ss.ssss"',
                    // Coordinates in this format
                    'coordinates' => 'N15°10\'45.84", E39°46\'56.28"',
                ],
                [
                    // Format description
                    'format' => 'dd°mm\'ss.ssss"[N,S], ddd°mm\'ss.ssss"[E,W]',
                    // Coordinates in this format
                    'coordinates' => '15°10\'45.84"N, 39°46\'56.28"E',
                ],
                [
                    // Format description
                    'format' => '[-]dd mm ss.ssss, [-]ddd mm ss.ssss',
                    // Coordinates in this format
                    'coordinates' => '15 10 45.84, 39 46 56.28',
                ],
                [
                    // Format description
                    'format' => 'ISO 6709: [-]ddmmss.ssss, [-]dddmmss.ssss',
                    // Coordinates in this format
                    'coordinates' => '151045.84, 394656.28',
                ],
                [
                    // Format description
                    'format' => '[-]dd.dddd [-]ddd.dddd',
                    // Coordinates in this format
                    'coordinates' => '15.1794 39.7823',
                ],
                [
                    // Format description
                    'format' => '[-]dd.ddddd°,[-]ddd.ddddd° (Dec Degs)',
                    // Coordinates in this format
                    'coordinates' => '15.17940°, 39.78230°',
                ],
                [
                    // Format description
                    'format' => 'dd.ddddd[N,S]ddd.ddddd[E,W] (Dec Degs Micro)',
                    // Coordinates in this format
                    'coordinates' => '15.1794000N39.7823000E',
                ],
                [
                    // Format description
                    'format' => 'ddmm.mmmm[N,S]dddmm.mmmm[E,W] (Dec Min)',
                    // Coordinates in this format
                    'coordinates' => '1510.764N03946.938E',
                ],
                [
                    // Format description
                    'format' => 'dd°mm\'ss.sss"[N,S], ddd°mm\'ss.sss"[E,W] (Deg Min Secs)',
                    // Coordinates in this format
                    'coordinates' => '15°10\'45.84"N, 39°46\'56.28"E',
                ],
                [
                    // Format description
                    'format' => 'ddmmss.sss[N,S]dddmmss.sss[E,W] (Deg Mins Secs)',
                    // Coordinates in this format
                    'coordinates' => '151045.84N0394656.28E',
                ],
                [
                    // Format description
                    'format' => '[N,S] dd mm.mmm [E,W] ddd mm.mmm',
                    // Coordinates in this format
                    'coordinates' => 'N 15 10.764 E 39 46.938',
                ],
                [
                    // Format description
                    'format' => 'dd:mm:ss[N,S],ddd:mm:ss[E,W]',
                    // Coordinates in this format
                    'coordinates' => '15:10:45N,39:46:56E',
                ],
                [
                    // Format description
                    'format' => 'dd:mm:ss.ss[N,S] ddd:mm:ss.ss[E,W]',
                    // Coordinates in this format
                    'coordinates' => '15:10:45.84N 39:46:56.28E',
                ],
                [
                    // Format description
                    'format' => 'dd°mm\'ss"[N,S] ddd°mm\'ss"[E,W]',
                    // Coordinates in this format
                    'coordinates' => '15°10\'45"N 39°46\'56"E',
                ],
                [
                    // Format description
                    'format' => '[-]dd°mm\'ss" [-]ddd°mm\'ss"',
                    // Coordinates in this format
                    'coordinates' => '15°10\'45" 39°46\'56"',
                ],
                [
                    // Format description
                    'format' => 'dd mm\' ss" [N,S] ddd mm\' ss" [E,W]',
                    // Coordinates in this format
                    'coordinates' => '15d 10\' 45" N 39d 46\' 56" E',
                ],
                [
                    // Format description
                    'format' => 'dd.dddd[N,S] ddd.dddd[E,W]',
                    // Coordinates in this format
                    'coordinates' => '15.1794N 39.7823E',
                ],
                [
                    // Format description
                    'format' => '[-]dd° mm.mmmmm [-]ddd° mm.mmmmm',
                    // Coordinates in this format
                    'coordinates' => '15° 10.764, 39° 46.938',
                ],
                [
                    // Format description
                    'format' => '[-] mmmm.mmmmm, [-] mmmm.mmmmm',
                    // Coordinates in this format
                    'coordinates' => '1510.764, 3946.938',
                ],
                [
                    // Format description
                    'format' => 'mmmm.mmmmm[N,S] mmmm.mmmmm[E,W]',
                    // Coordinates in this format
                    'coordinates' => '1510.764N, 3946.938E',
                ],
            ],
        ];

        $this->geographical = json_decode($this->geographical(), true);

        Builder::country($this);
    }

    public function geographical()
    {
        return '{
  "type": "FeatureCollection",
  "features": [
    {
      "type": "Feature",
      "properties": { "cca2": "er" },
      "geometry": {
        "type": "MultiPolygon",
        "coordinates": [
          [
            [
              [40.087219, 15.851665],
              [40.138885, 15.807777],
              [40.141388, 15.80361],
              [40.141937, 15.797499],
              [40.135826, 15.752499],
              [40.128609, 15.731667],
              [40.12888, 15.718611],
              [40.13222, 15.701944],
              [40.142494, 15.67139],
              [40.146385, 15.66166],
              [40.151665, 15.653332],
              [40.161942, 15.642776],
              [40.166107, 15.639721],
              [40.171661, 15.638054],
              [40.232216, 15.626389],
              [40.237495, 15.628054],
              [40.241104, 15.631388],
              [40.251938, 15.655554],
              [40.233887, 15.665833],
              [40.22361, 15.669722],
              [40.219444, 15.672499],
              [40.2175, 15.6775],
              [40.219162, 15.682777],
              [40.221939, 15.68694],
              [40.229439, 15.693333],
              [40.238052, 15.69861],
              [40.24889, 15.701666],
              [40.255272, 15.702221],
              [40.262497, 15.701944],
              [40.283882, 15.701111],
              [40.29639, 15.699165],
              [40.313332, 15.694443],
              [40.402771, 15.638332],
              [40.409721, 15.618055],
              [40.418884, 15.58],
              [40.419441, 15.573889],
              [40.414993, 15.57278],
              [40.395828, 15.57278],
              [40.326942, 15.57472],
              [40.252777, 15.589722],
              [40.124161, 15.606943],
              [40.119995, 15.60583],
              [40.116943, 15.601944],
              [40.11222, 15.592499],
              [40.109161, 15.58861],
              [40.104996, 15.585833],
              [40.088608, 15.585833],
              [40.049438, 15.588055],
              [39.983887, 15.600832],
              [39.977776, 15.605],
              [39.96, 15.622221],
              [39.957222, 15.626389],
              [39.95122, 15.67311],
              [39.95122, 15.676443],
              [39.953552, 15.678444],
              [39.956886, 15.67911],
              [39.960716, 15.678444],
              [39.963722, 15.677111],
              [39.98305, 15.664999],
              [40.024719, 15.643055],
              [40.034439, 15.638611],
              [40.04028, 15.639721],
              [40.059166, 15.648611],
              [40.075829, 15.658333],
              [40.079437, 15.661388],
              [40.080551, 15.6675],
              [40.060829, 15.689444],
              [40.019165, 15.731667],
              [40.00916, 15.736111],
              [39.98027, 15.741943],
              [39.97388, 15.742777],
              [39.967499, 15.742222],
              [39.955662, 15.742999],
              [39.949497, 15.736666],
              [39.946499, 15.736333],
              [39.943497, 15.736832],
              [39.934, 15.7435],
              [39.932831, 15.745166],
              [39.921661, 15.75361],
              [39.92139, 15.759722],
              [39.92722, 15.783609],
              [39.92889, 15.78889],
              [39.982498, 15.814348],
              [40.025551, 15.822222],
              [40.032776, 15.821943],
              [40.03694, 15.82472],
              [40.03999, 15.82861],
              [40.04111, 15.834721],
              [40.039719, 15.840277],
              [40, 15.885777],
              [40.008331, 15.88666],
              [40.014717, 15.88583],
              [40.01944, 15.88361],
              [40.087219, 15.851665]
            ]
          ],
          [
            [
              [40.113327, 16.055832],
              [40.114441, 15.998888],
              [40.11305, 15.99361],
              [40.109993, 15.98972],
              [40.104721, 15.987778],
              [40.09333, 15.986944],
              [40.059715, 15.994999],
              [40, 16.0135],
              [39.984161, 16.01083],
              [39.979721, 16.012218],
              [39.979439, 16.01833],
              [39.979721, 16.02528],
              [39.981384, 16.03083],
              [39.985832, 16.04],
              [39.988884, 16.044167],
              [40.024994, 16.09333],
              [40.030273, 16.095276],
              [40.03666, 16.095554],
              [40.060272, 16.085278],
              [40.111382, 16.060833],
              [40.113327, 16.055832]
            ]
          ],
          [
            [
              [38.797775, 17.65333],
              [38.895, 17.456944],
              [38.945274, 17.348053],
              [39.001938, 17.18861],
              [39.039162, 17.052498],
              [39.09833, 16.862778],
              [39.11555, 16.826942],
              [39.13499, 16.78],
              [39.138885, 16.770554],
              [39.160271, 16.67916],
              [39.161659, 16.637497],
              [39.160828, 16.573055],
              [39.164162, 16.544441],
              [39.172493, 16.483055],
              [39.173607, 16.477776],
              [39.177498, 16.468052],
              [39.187492, 16.45055],
              [39.192772, 16.396942],
              [39.190277, 16.385559],
              [39.20222, 16.34333],
              [39.204163, 16.288609],
              [39.211388, 16.22889],
              [39.214722, 16.205555],
              [39.229439, 16.127499],
              [39.269165, 15.988609],
              [39.271111, 15.983889],
              [39.305832, 15.901943],
              [39.312492, 15.894722],
              [39.36555, 15.849722],
              [39.423607, 15.769722],
              [39.449715, 15.650276],
              [39.448051, 15.605276],
              [39.441666, 15.60611],
              [39.435829, 15.605],
              [39.43222, 15.601665],
              [39.431389, 15.595554],
              [39.45694, 15.525555],
              [39.459717, 15.521111],
              [39.467216, 15.514721],
              [39.479164, 15.51222],
              [39.486382, 15.51194],
              [39.50417, 15.515554],
              [39.51944, 15.521111],
              [39.534164, 15.534443],
              [39.54277, 15.540277],
              [39.55555, 15.538332],
              [39.571106, 15.532221],
              [39.580826, 15.527777],
              [39.585, 15.525],
              [39.609444, 15.493889],
              [39.617218, 15.48139],
              [39.69055, 15.315554],
              [39.712494, 15.262777],
              [39.714439, 15.257776],
              [39.713333, 15.244999],
              [39.701942, 15.192499],
              [39.698608, 15.18167],
              [39.692772, 15.138054],
              [39.693054, 15.131943],
              [39.694443, 15.126389],
              [39.696388, 15.121387],
              [39.714996, 15.091944],
              [39.721107, 15.084166],
              [39.76778, 15.062777],
              [39.774162, 15.063055],
              [39.800278, 15.071665],
              [39.815277, 15.0775],
              [39.81833, 15.081388],
              [39.85361, 15.166388],
              [39.85611, 15.178055],
              [39.85527, 15.190277],
              [39.843048, 15.225832],
              [39.82222, 15.27305],
              [39.81694, 15.281666],
              [39.811943, 15.283888],
              [39.806389, 15.282499],
              [39.803329, 15.27861],
              [39.801109, 15.273888],
              [39.798332, 15.262499],
              [39.796104, 15.2575],
              [39.793053, 15.253611],
              [39.787498, 15.252499],
              [39.78388, 15.25611],
              [39.781944, 15.260832],
              [39.77833, 15.2775],
              [39.769722, 15.377777],
              [39.770271, 15.391388],
              [39.771111, 15.397499],
              [39.773331, 15.402222],
              [39.796387, 15.442499],
              [39.799438, 15.446665],
              [39.803055, 15.44972],
              [39.84, 15.471109],
              [39.85527, 15.47805],
              [39.88139, 15.489443],
              [39.989716, 15.389444],
              [40.050278, 15.289444],
              [40.047775, 15.284443],
              [40.02694, 15.242222],
              [40.024719, 15.237499],
              [40.041664, 15.201111],
              [40.068886, 15.172777],
              [40.073051, 15.169998],
              [40.086105, 15.15639],
              [40.158333, 15.029722],
              [40.158607, 15.023333],
              [40.157776, 15.017221],
              [40.15222, 15.001665],
              [40.152496, 14.99],
              [40.162498, 14.979443],
              [40.17083, 14.97361],
              [40.179993, 14.968611],
              [40.290276, 14.912777],
              [40.301109, 14.911665],
              [40.30971, 14.916943],
              [40.33861, 14.943888],
              [40.361938, 14.964998],
              [40.422218, 14.996111],
              [40.451942, 15.008055],
              [40.455276, 15.00861],
              [40.460274, 15.006388],
              [40.593887, 14.946388],
              [40.673882, 14.904165],
              [40.72916, 14.836111],
              [40.732216, 14.825554],
              [40.732498, 14.799444],
              [40.728607, 14.789444],
              [40.80722, 14.705555],
              [40.849442, 14.716665],
              [40.882774, 14.71611],
              [41.17055, 14.6325],
              [41.173882, 14.628887],
              [41.310272, 14.476944],
              [41.37861, 14.373333],
              [41.449165, 14.271666],
              [41.51194, 14.202499],
              [41.611938, 14.06778],
              [41.648888, 14.00861],
              [41.655273, 13.995832],
              [41.659164, 13.986111],
              [41.662216, 13.97555],
              [41.66555, 13.95917],
              [41.67333, 13.93944],
              [41.676666, 13.93583],
              [41.685555, 13.930832],
              [41.720276, 13.91555],
              [41.84916, 13.871666],
              [41.85611, 13.871111],
              [41.894997, 13.870277],
              [41.909164, 13.874998],
              [41.95444, 13.85861],
              [41.958611, 13.855833],
              [41.984718, 13.826387],
              [42.058609, 13.729166],
              [42.067772, 13.697777],
              [42.090271, 13.678333],
              [42.18444, 13.576666],
              [42.18861, 13.57361],
              [42.221382, 13.550278],
              [42.227776, 13.55055],
              [42.231384, 13.553888],
              [42.219444, 13.63278],
              [42.217773, 13.6375],
              [42.21235, 13.642534],
              [42.196434, 13.651423],
              [42.186661, 13.656111],
              [42.180275, 13.65583],
              [42.169716, 13.652777],
              [42.16333, 13.652498],
              [42.15833, 13.654722],
              [42.156387, 13.659721],
              [42.168884, 13.668055],
              [42.174721, 13.669167],
              [42.178886, 13.668888],
              [42.222496, 13.636665],
              [42.283333, 13.575554],
              [42.285828, 13.571388],
              [42.294998, 13.55305],
              [42.339722, 13.4275],
              [42.344444, 13.405277],
              [42.348885, 13.350277],
              [42.36805, 13.223055],
              [42.374718, 13.214998],
              [42.38444, 13.210554],
              [42.437775, 13.192221],
              [42.448883, 13.188889],
              [42.455276, 13.188055],
              [42.474998, 13.18861],
              [42.487221, 13.189999],
              [42.49721, 13.193609],
              [42.501663, 13.19611],
              [42.57527, 13.161943],
              [42.71666, 13.047499],
              [42.726662, 13.036943],
              [42.731941, 13.02833],
              [42.73555, 13.01861],
              [42.758606, 12.9425],
              [42.771942, 12.894444],
              [42.804718, 12.845],
              [42.874161, 12.811388],
              [42.883888, 12.80694],
              [42.889999, 12.805832],
              [42.927773, 12.799999],
              [42.944717, 12.801943],
              [42.974998, 12.81278],
              [42.98305, 12.81944],
              [42.986107, 12.823332],
              [42.987778, 12.828888],
              [42.988052, 12.84861],
              [42.99222, 12.87611],
              [42.993332, 12.882221],
              [42.997215, 12.892221],
              [43.00222, 12.894165],
              [43.007774, 12.8925],
              [43.012772, 12.89],
              [43.02, 12.88361],
              [43.073326, 12.836388],
              [43.079437, 12.828609],
              [43.085831, 12.814444],
              [43.09389, 12.795555],
              [43.11805, 12.729166],
              [43.121384, 12.70833],
              [42.929718, 12.623888],
              [42.821938, 12.539721],
              [42.808052, 12.525833],
              [42.80639, 12.52055],
              [42.805832, 12.513611],
              [42.805275, 12.5],
              [42.805832, 12.49361],
              [42.808609, 12.48333],
              [42.813889, 12.474722],
              [42.82222, 12.455832],
              [42.822495, 12.451111],
              [42.821663, 12.444721],
              [42.81916, 12.440277],
              [42.816109, 12.43611],
              [42.808884, 12.429722],
              [42.751938, 12.388887],
              [42.747772, 12.386389],
              [42.69861, 12.363888],
              [42.649162, 12.398611],
              [42.620277, 12.430832],
              [42.589722, 12.466665],
              [42.586937, 12.468887],
              [42.535828, 12.504166],
              [42.501389, 12.516666],
              [42.48481, 12.52185],
              [42.45611, 12.529165],
              [42.399719, 12.469721],
              [42.364716, 12.504444],
              [42.35139, 12.518888],
              [42.344719, 12.52667],
              [42.32611, 12.549444],
              [42.310829, 12.56861],
              [42.30222, 12.58083],
              [42.294441, 12.593611],
              [42.289993, 12.60278],
              [42.262772, 12.650555],
              [42.2175, 12.721943],
              [42.211388, 12.729721],
              [42.198051, 12.742777],
              [42.190552, 12.74944],
              [42.159164, 12.774166],
              [42.146942, 12.782776],
              [42.125832, 12.797499],
              [42.112221, 12.804998],
              [42.097771, 12.811943],
              [42.076942, 12.819721],
              [42.056107, 12.827499],
              [42.040276, 12.833055],
              [42.024162, 12.83861],
              [42.010277, 12.842499],
              [42, 12.846388],
              [41.986107, 12.85389],
              [41.97805, 12.859722],
              [41.970551, 12.866943],
              [41.957222, 12.881388],
              [41.95111, 12.88917],
              [41.929443, 12.922499],
              [41.924164, 12.930832],
              [41.916939, 12.944166],
              [41.912498, 12.953333],
              [41.843864, 13.098539],
              [41.843887, 13.103333],
              [41.84222, 13.108332],
              [41.818886, 13.153332],
              [41.814163, 13.16222],
              [41.798607, 13.187777],
              [41.781387, 13.211943],
              [41.75694, 13.23639],
              [41.615555, 13.353611],
              [41.59916, 13.36528],
              [41.56694, 13.386665],
              [41.549721, 13.397778],
              [41.532494, 13.408609],
              [41.5236, 13.413887],
              [41.428055, 13.472221],
              [41.240555, 13.592499],
              [41.232498, 13.59833],
              [41.224998, 13.605],
              [41.212219, 13.61972],
              [41.178329, 13.681665],
              [41.1325, 13.766109],
              [41.11555, 13.796944],
              [41.10305, 13.818609],
              [41.097771, 13.827221],
              [41.077217, 13.860277],
              [41.068604, 13.872776],
              [41.047218, 13.9],
              [41.008049, 13.944721],
              [40.929161, 14.032221],
              [40.909439, 14.05222],
              [40.814438, 14.145277],
              [40.801941, 14.15416],
              [40.797218, 14.156387],
              [40.776939, 14.16472],
              [40.748886, 14.172777],
              [40.693604, 14.182499],
              [40.66333, 14.187222],
              [40.649994, 14.188332],
              [40.637215, 14.190277],
              [40.614166, 14.195833],
              [40.592499, 14.20305],
              [40.552773, 14.220276],
              [40.468887, 14.261389],
              [40.437775, 14.27972],
              [40.429443, 14.28555],
              [40.392776, 14.31861],
              [40.378326, 14.331944],
              [40.349716, 14.359165],
              [40.322777, 14.387777],
              [40.301384, 14.405832],
              [40.271111, 14.424721],
              [40.228058, 14.443506],
              [40.179432, 14.457955],
              [40.15666, 14.464722],
              [40.12555, 14.469999],
              [40.111382, 14.47055],
              [40.104996, 14.469999],
              [40.018883, 14.458887],
              [39.954994, 14.434721],
              [39.950554, 14.431944],
              [39.940552, 14.42833],
              [39.93471, 14.42722],
              [39.920555, 14.427776],
              [39.908607, 14.430277],
              [39.898888, 14.434721],
              [39.89055, 14.440554],
              [39.883049, 14.446943],
              [39.85583, 14.475277],
              [39.853333, 14.47972],
              [39.84166, 14.488888],
              [39.789444, 14.513611],
              [39.776108, 14.514721],
              [39.764717, 14.512499],
              [39.74638, 14.502777],
              [39.731384, 14.49722],
              [39.720551, 14.494165],
              [39.701385, 14.492777],
              [39.688889, 14.494444],
              [39.61388, 14.51033],
              [39.531387, 14.565554],
              [39.48555, 14.513611],
              [39.481941, 14.510277],
              [39.47749, 14.507776],
              [39.38166, 14.484999],
              [39.37028, 14.482777],
              [39.263054, 14.47361],
              [39.209717, 14.485554],
              [39.190552, 14.54194],
              [39.188606, 14.546665],
              [39.18388, 14.555832],
              [39.173332, 14.57278],
              [39.152222, 14.599998],
              [39.135277, 14.617777],
              [39.118744, 14.629391],
              [39.101387, 14.640276],
              [39.086662, 14.64694],
              [39.024021, 14.655162],
              [39.017776, 14.599443],
              [39.013885, 14.589167],
              [39.010826, 14.585278],
              [39.007217, 14.581944],
              [38.965553, 14.54611],
              [38.893051, 14.500832],
              [38.795555, 14.46972],
              [38.570274, 14.428055],
              [38.552498, 14.425],
              [38.462776, 14.41639],
              [38.44944, 14.4175],
              [38.443886, 14.419167],
              [38.43972, 14.421944],
              [38.340828, 14.51166],
              [38.33416, 14.51861],
              [38.331383, 14.522778],
              [38.3275, 14.533333],
              [38.306938, 14.59417],
              [38.265, 14.675833],
              [38.261665, 14.679443],
              [38.253326, 14.684999],
              [38.236382, 14.689722],
              [38.19971, 14.695],
              [38.192772, 14.695276],
              [38.187775, 14.693333],
              [38.184166, 14.689999],
              [38.179718, 14.687498],
              [38.17472, 14.685555],
              [38.149719, 14.682777],
              [38.137772, 14.683887],
              [38.04555, 14.719721],
              [38.035553, 14.724165],
              [38.018326, 14.734999],
              [37.981941, 14.7725],
              [37.979439, 14.776667],
              [37.97805, 14.782221],
              [37.97916, 14.795277],
              [37.977493, 14.80694],
              [37.955826, 14.837776],
              [37.929993, 14.867498],
              [37.92305, 14.87444],
              [37.911385, 14.88361],
              [37.83833, 14.715277],
              [37.68444, 14.359999],
              [37.57221, 14.102253],
              [37.541939, 14.107777],
              [37.53222, 14.11222],
              [37.52444, 14.11861],
              [37.433052, 14.268055],
              [37.393883, 14.35222],
              [37.388611, 14.360554],
              [37.33694, 14.426666],
              [37.301941, 14.448332],
              [37.291664, 14.451944],
              [37.278885, 14.453609],
              [37.202499, 14.453054],
              [37.196663, 14.451944],
              [37.136108, 14.43528],
              [37.12722, 14.429998],
              [37.123604, 14.426666],
              [37.111938, 14.41055],
              [37.10139, 14.39305],
              [37.0925, 14.373888],
              [37.090828, 14.36861],
              [37.089722, 14.35555],
              [37.091942, 14.337776],
              [37.092216, 14.31778],
              [37.088882, 14.306944],
              [37.084442, 14.297499],
              [37.07861, 14.289444],
              [37.071106, 14.282776],
              [37.06694, 14.280277],
              [37.008888, 14.26194],
              [37.003326, 14.260832],
              [36.99638, 14.260832],
              [36.99055, 14.262499],
              [36.961388, 14.275555],
              [36.953606, 14.281944],
              [36.954163, 14.295555],
              [36.949997, 14.298611],
              [36.945, 14.300833],
              [36.88611, 14.320276],
              [36.824997, 14.328054],
              [36.736382, 14.327221],
              [36.64416, 14.317221],
              [36.638329, 14.316111],
              [36.56472, 14.292776],
              [36.559715, 14.290833],
              [36.556107, 14.287498],
              [36.553329, 14.283333],
              [36.542816, 14.262053],
              [36.541664, 14.278055],
              [36.515549, 14.450277],
              [36.452217, 15.06861],
              [36.443283, 15.149952],
              [36.47972, 15.180277],
              [36.54277, 15.234722],
              [36.56555, 15.302776],
              [36.61055, 15.437222],
              [36.626106, 15.489443],
              [36.671661, 15.659166],
              [36.694717, 15.744165],
              [36.69778, 15.753054],
              [36.763611, 15.808054],
              [36.82611, 15.977499],
              [36.889442, 16.14666],
              [36.915833, 16.215],
              [36.920273, 16.22444],
              [36.931389, 16.234444],
              [36.9575, 16.25],
              [36.965553, 16.25611],
              [36.971382, 16.26416],
              [36.973053, 16.269444],
              [36.97416, 16.282497],
              [36.97361, 16.288609],
              [36.95694, 16.425278],
              [36.954994, 16.430832],
              [36.949715, 16.439163],
              [36.928055, 16.459442],
              [36.906105, 16.486664],
              [36.899437, 16.500553],
              [36.896942, 16.511665],
              [36.896385, 16.51778],
              [36.89833, 16.536942],
              [36.910553, 16.63555],
              [36.912216, 16.641109],
              [36.916664, 16.650555],
              [36.931664, 16.67139],
              [36.946938, 16.683887],
              [36.959999, 16.691666],
              [36.969444, 16.69611],
              [36.98055, 16.706108],
              [36.988884, 16.72555],
              [37.006104, 16.771664],
              [37.009163, 16.782497],
              [37.010277, 16.79555],
              [37.018326, 17.010555],
              [37.014999, 17.02083],
              [37.009438, 17.029163],
              [36.993607, 17.048054],
              [36.99166, 17.052776],
              [36.99055, 17.058331],
              [36.99139, 17.06444],
              [36.995827, 17.073887],
              [37.02611, 17.077221],
              [37.104439, 17.051109],
              [37.112221, 17.04472],
              [37.135826, 17.02083],
              [37.145554, 17.01639],
              [37.151939, 17.015553],
              [37.157776, 17.01527],
              [37.25222, 17.024998],
              [37.2625, 17.028889],
              [37.381386, 17.04305],
              [37.423286, 17.034214],
              [37.486107, 17.110554],
              [37.520576, 17.195801],
              [37.52861, 17.231941],
              [37.532501, 17.24995],
              [37.531662, 17.255554],
              [37.53, 17.260277],
              [37.520271, 17.277775],
              [37.50996, 17.31556],
              [37.508888, 17.32],
              [37.512497, 17.32333],
              [37.516937, 17.325832],
              [37.583328, 17.34861],
              [37.59416, 17.351665],
              [37.623055, 17.357498],
              [37.74472, 17.381943],
              [37.895554, 17.441387],
              [37.933884, 17.458609],
              [37.964722, 17.498055],
              [37.96833, 17.501389],
              [38.028053, 17.537777],
              [38.041382, 17.54555],
              [38.05583, 17.551941],
              [38.06166, 17.553055],
              [38.07527, 17.551941],
              [38.087219, 17.549442],
              [38.092216, 17.547222],
              [38.095551, 17.543888],
              [38.09694, 17.53833],
              [38.13555, 17.498886],
              [38.248329, 17.528053],
              [38.25777, 17.5325],
              [38.26194, 17.535275],
              [38.36805, 17.662777],
              [38.370277, 17.666111],
              [38.449165, 17.851109],
              [38.458611, 17.87111],
              [38.505829, 17.91333],
              [38.60069, 17.994881],
              [38.611938, 17.961388],
              [38.672218, 17.847221],
              [38.707771, 17.781387],
              [38.713333, 17.77333],
              [38.749718, 17.727219],
              [38.797775, 17.65333]
            ]
          ]
        ]
      }
    }
  ]
}
';
    }
}
