<?php

namespace Lwwcas\LaravelCountries\Database\Seeders\Countries;

use Illuminate\Database\Seeder;
use Lwwcas\LaravelCountries\Database\Seeders\Builder;

class AL_Albania extends Seeder
{

    /**
     * Attribute that defines the language of countries
     *
     * @var string
     */
    public $lang = 'en';

    /**
     * Attribute that defines the language of countries
     *
     * @var string
     */
    public $region = 'europe';

    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        // Set the country's common name
        $this->name = 'Albania';

        // Set the country's official name
        $this->official_name = 'Republic of Albania';

        // Set the ISO 3166-1 alpha-2 code
        $this->iso_alpha_2 = 'AL';

        // Set the ISO 3166-1 alpha-3 code
        $this->iso_alpha_3 = 'ALB';

        // Set the ISO 3166-1 numeric code (usually three digits)
        $this->iso_numeric = '008';

        // Set the international dialing code
        $this->international_phone = '355';

        // Define the languages spoken in the country
        $this->languages = ['sq']; // Albanian is the official language

        // Define the top-level domain(s)
        $this->tld = ['.al'];

        // Define alternative top-level domains
        $this->alternative_tlds = []; // No significant alternative TLDs

        // Define average internet speeds
        $this->internet_speed = [
            // Average fixed broadband speed
            'average_speed_fixed' => '30 Mbps',
            // Average mobile connection speed
            'average_speed_mobile' => '20 Mbps',
        ];

        // Define internet penetration rate
        $this->internet_penetration = '72%'; // Percentage of the population with internet access

        // Set the national cybersecurity agency
        $this->cybersecurity_agency = 'National Authority for Electronic Certification and Cyber Security'; // Government body responsible for cybersecurity

        // List popular web technologies in Albania
        $this->popular_technologies = ['PHP', 'JavaScript', 'WordPress', 'Laravel']; // Popular web technologies in Albania

        // Set the WMO (World Meteorological Organization) country code
        $this->wmo = 'AB';

        // Set the GeoName ID for geographical data reference
        $this->geoname_id = '783754';

        // Define the currency information
        $this->currency = [
            // Currency name
            'name' => 'Albanian lek',
            // Currency code
            'code' => 'ALL',
            // Currency symbol
            'symbol' => 'L',
            // Main unit
            'main_unit' => 'lek',
            // Subunit
            'sub_unit' => 'qindarkë',
            // Subunit to main unit conversion
            'sub_unit_to_unit' => 100,
            // Banknotes in circulation
            'banknotes' => ['200', '500', '1000', '2000', '5000'],
            // Main coins
            'coins_main' => ['5', '10', '20', '50', '100'],
            // Subunit coins
            'coins_sub' => ['1', '5', '10', '20', '50'],
        ];

        // Set the population
        $this->population = 2837743; // As of recent estimates

        // Set the area in square kilometers
        $this->area = 28748; // km²

        // Set the capital city
        $this->capital = 'Tirana';

        // Define the time zone(s) using IANA time zone identifiers
        $this->timezones = [
            // Time zone identifier
            'timezone_id' => 'Europe/Tirane',
            // Standard time offset
            'standard_time' => 'UTC+1', // Central European Time
            // Daylight saving time offset
            'daylight_saving_time' => 'UTC+2', // Central European Summer Time
        ];

        // Set the independence day
        $this->independence_day = '1912-11-28'; // Date of Independence

        // List international organizations the country is part of
        $this->international_organizations = ['United Nations', 'NATO', 'Council of Europe', 'WTO', 'OSCE'];

        // Set the GDP in billions of USD
        $this->gdp = 15.28; // GDP in billions of USD

        // List the predominant religions
        $this->religions = ['Islam', 'Christianity']; // Predominant religions

        // Describe the form of government
        $this->government = 'Unitary parliamentary constitutional republic';

        // Set the national sport
        $this->national_sport = 'Football (Soccer)';

        // Define bordering countries
        $this->borders = [
            ['name' => 'Montenegro', 'iso_alpha_2' => 'ME'],
            ['name' => 'Kosovo', 'iso_alpha_2' => 'XK'],
            ['name' => 'North Macedonia', 'iso_alpha_2' => 'MK'],
            ['name' => 'Greece', 'iso_alpha_2' => 'GR'],
        ];

        // Define the country's flag emoji representation
        $this->emoji = [
            // Native emoji image
            'img' => '🇦🇱',
            // Unicode code points
            'uCode' => 'U+1F1E6 U+1F1F1',
            // HTML entities
            'html' => '&#x1F1E6;&#x1F1F1;',
            // CSS entities
            'css' => '\\1F1E6\\1F1F1',
            // Decimal Unicode entities
            'decimal' => '&#127462;&#127473;',
            // Direct UTF-8 string
            'utf8' => '🇦🇱',
            // UTF-16 surrogate pairs
            'utf16' => '\uD83C\uDDE6\uD83C\uDDF1',
            // Shortcode in Emojione, Slack
            'shortcode' => ':flag-al:',
            // Hexadecimal entity
            'hex' => '&#x1F1E6;&#x1F1F1;',
        ];

        // Define the colors of the country's flag with accurate color values
        $this->flag_colors = [
            [
                // Color name
                'name' => 'Red',
                // Web color name
                'web_name' => 'red',
                // Hexadecimal color code
                'hex' => '#E41B17',
                // RGB color values
                'rgb' => '228,27,23',
                // CMYK color values
                'cmyk' => '0,88,90,11',
                // HSL color values
                'hsl' => '2,81%,49%',
                // HSV color values
                'hsv' => '2,90%,89%',
                // Pantone color code
                'pantone' => 'Pantone 1795 C',
                // Contrast color for readability
                'contrast' => '#000000',
            ],
            [
                // Color name
                'name' => 'Black',
                // Web color name
                'web_name' => 'black',
                // Hexadecimal color code
                'hex' => '#000000',
                // RGB color values
                'rgb' => '0,0,0',
                // CMYK color values
                'cmyk' => '0,0,0,100',
                // HSL color values
                'hsl' => '0,0%,0%',
                // HSV color values
                'hsv' => '0,0%,0%',
                // Pantone color code
                'pantone' => 'Pantone Black C',
                // Contrast color for readability
                'contrast' => '#FFFFFF',
            ],
        ];

        // Define the geographical coordinates
        $this->coordinates = [
            // Latitude in decimal degrees
            'latitude' => '41.3275',
            // Longitude in decimal degrees
            'longitude' => '19.8187',
            // Degrees with decimal
            'dd' => '41.3275° N, 19.8187° E',
            // Degrees, minutes, seconds
            'dms' => '41°19\'39.00" N, 19°49\'07.32" E',
            // Degrees and decimal minutes
            'dm' => '41°19.650\' N, 19°49.122\' E',
            // Various GPS formats
            'gps' => [
                [
                    // Format description
                    'format' => '[N,S]dd°mm\'ss.ssss", [E,W]dd°mm\'ss.ssss"',
                    // Coordinates in this format
                    'coordinates' => 'N41°19\'39.00", E19°49\'07.32"',
                ],
                [
                    // Format description
                    'format' => 'dd°mm\'ss.ssss"[N,S], dd°mm\'ss.ssss"[E,W]',
                    // Coordinates in this format
                    'coordinates' => '41°19\'39.00"N, 19°49\'07.32"E',
                ],
                [
                    // Format description
                    'format' => '[-]dd mm ss.ssss, [-]dd mm ss.ssss',
                    // Coordinates in this format
                    'coordinates' => '41 19 39.00, 19 49 07.32',
                ],
                [
                    // Format description
                    'format' => 'ISO 6709: [-]ddmmss.ssss, [-]dddmmss.ssss',
                    // Coordinates in this format
                    'coordinates' => '411939.00, 0194907.32',
                ],
                [
                    // Format description
                    'format' => '[-]dd.dddd [-]dd.dddd',
                    // Coordinates in this format
                    'coordinates' => '41.3275 19.8187',
                ],
                [
                    // Format description
                    'format' => '[-]dd.ddddd°,[-]dd.ddddd° (Dec Degs)',
                    // Coordinates in this format
                    'coordinates' => '41.32750°,19.81870°',
                ],
                [
                    // Format description
                    'format' => 'dd.ddddd[N,S]dd.ddddd[E,W] (Dec Degs Micro)',
                    // Coordinates in this format
                    'coordinates' => '41.3275000N19.8187000E',
                ],
                [
                    // Format description
                    'format' => 'ddmm.mmmm[N,S]ddmm.mmmm[E,W] (Dec Min)',
                    // Coordinates in this format
                    'coordinates' => '4119.650N01949.122E',
                ],
                [
                    // Format description
                    'format' => 'dd°mm\'ss.sss"[N,S], dd°mm\'ss.sss"[E,W] (Deg Min Secs)',
                    // Coordinates in this format
                    'coordinates' => '41°19\'39.00"N, 19°49\'07.32"E',
                ],
                [
                    // Format description
                    'format' => 'ddmmss.sss[N,S]ddmmss.sss[E,W] (Deg Mins Secs)',
                    // Coordinates in this format
                    'coordinates' => '411939.00N0194907.32E',
                ],
                [
                    // Format description
                    'format' => '[N,S] dd mm.mmm [E,W] dd mm.mmm',
                    // Coordinates in this format
                    'coordinates' => 'N 41 19.650 E 19 49.122',
                ],
                [
                    // Format description
                    'format' => 'dd:mm:ss[N,S],dd:mm:ss[E,W]',
                    // Coordinates in this format
                    'coordinates' => '41:19:39N,19:49:07E',
                ],
                [
                    // Format description
                    'format' => 'dd:mm:ss.ss[N,S] dd:mm:ss.ss[E,W]',
                    // Coordinates in this format
                    'coordinates' => '41:19:39.00N 19:49:07.32E',
                ],
                [
                    // Format description
                    'format' => 'dd°mm\'ss"[N,S] dd°mm\'ss"[E,W]',
                    // Coordinates in this format
                    'coordinates' => '41°19\'39"N 19°49\'07"E',
                ],
                [
                    // Format description
                    'format' => '[-]dd°mm\'ss" [-]dd°mm\'ss"',
                    // Coordinates in this format
                    'coordinates' => '41°19\'39" 19°49\'07"',
                ],
                [
                    // Format description
                    'format' => 'dd mm\' ss" [N,S] dd mm\' ss" [E,W]',
                    // Coordinates in this format
                    'coordinates' => '41d 19\' 39" N 19d 49\' 07" E',
                ],
                [
                    // Format description
                    'format' => 'dd.dddd[N,S] dd.dddd[E,W]',
                    // Coordinates in this format
                    'coordinates' => '41.3275N 19.8187E',
                ],
                [
                    // Format description
                    'format' => '[-]dd° mm.mmmmm [-]dd° mm.mmmmm',
                    // Coordinates in this format
                    'coordinates' => '41° 19.650, 19° 49.122',
                ],
                [
                    // Format description
                    'format' => '[-] mmmm.mmmmm, [-] mmmm.mmmmm',
                    // Coordinates in this format
                    'coordinates' => '4119.650, 1949.122',
                ],
                [
                    // Format description
                    'format' => 'mmmm.mmmmm[N,S] mmmm.mmmmm[E,W]',
                    // Coordinates in this format
                    'coordinates' => '4119.650N, 1949.122E',
                ],
            ],
        ];

        $this->geographical = json_decode($this->geographical(), true);

        Builder::country($this);
    }

    public function geographical()
    {
        return '{
  "type": "FeatureCollection",
  "features": [
    {
      "type": "Feature",
      "properties": { "cca2": "al" },
      "geometry": {
        "type": "Polygon",
        "coordinates": [
          [
            [19.436214, 41.021065],
            [19.450554, 41.059998],
            [19.513611, 41.251938],
            [19.513885, 41.256104],
            [19.51083, 41.274437],
            [19.509441, 41.277771],
            [19.504719, 41.287216],
            [19.492496, 41.300552],
            [19.482777, 41.30722],
            [19.467499, 41.310829],
            [19.452221, 41.309715],
            [19.442497, 41.406944],
            [19.519997, 41.57],
            [19.566666, 41.584999],
            [19.584721, 41.605827],
            [19.586941, 41.638611],
            [19.570829, 41.685268],
            [19.57972, 41.692497],
            [19.584164, 41.698883],
            [19.598053, 41.771111],
            [19.600277, 41.78833],
            [19.600555, 41.796661],
            [19.59972, 41.804161],
            [19.596664, 41.810555],
            [19.593887, 41.813049],
            [19.524757, 41.835243],
            [19.488888, 41.849442],
            [19.469997, 41.854996],
            [19.449165, 41.859718],
            [19.431664, 41.862221],
            [19.421108, 41.861938],
            [19.411388, 41.86055],
            [19.367771, 41.848999],
            [19.34333, 41.912498],
            [19.346943, 41.955551],
            [19.373333, 42.06694],
            [19.374603, 42.071487],
            [19.378052, 42.078606],
            [19.380981, 42.081001],
            [19.384163, 42.083611],
            [19.398888, 42.101387],
            [19.400276, 42.10527],
            [19.398331, 42.10833],
            [19.386387, 42.11805],
            [19.369999, 42.128883],
            [19.332222, 42.152496],
            [19.300278, 42.171104],
            [19.284443, 42.182777],
            [19.282497, 42.185555],
            [19.286942, 42.192215],
            [19.519722, 42.448608],
            [19.606388, 42.539444],
            [19.608887, 42.542496],
            [19.612499, 42.549995],
            [19.610832, 42.561104],
            [19.608608, 42.568054],
            [19.609444, 42.572495],
            [19.615833, 42.582497],
            [19.633331, 42.603333],
            [19.645832, 42.61805],
            [19.657219, 42.628326],
            [19.708885, 42.658882],
            [19.713055, 42.660553],
            [19.722775, 42.661942],
            [19.726665, 42.659996],
            [19.75333, 42.64222],
            [19.755833, 42.639717],
            [19.773052, 42.597496],
            [19.773609, 42.593605],
            [19.772778, 42.589996],
            [19.770275, 42.586937],
            [19.766109, 42.585548],
            [19.759998, 42.586388],
            [19.752499, 42.583054],
            [19.75, 42.579994],
            [19.746944, 42.546944],
            [19.746666, 42.543053],
            [19.765274, 42.504715],
            [19.773609, 42.497215],
            [19.790276, 42.486107],
            [19.79361, 42.483887],
            [19.801666, 42.48027],
            [19.822498, 42.471939],
            [19.919441, 42.512497],
            [19.935276, 42.518608],
            [20.049164, 42.561661],
            [20.053333, 42.563049],
            [20.057777, 42.563889],
            [20.064163, 42.563049],
            [20.071423, 42.560913],
            [20.0975, 42.536942],
            [20.106667, 42.529999],
            [20.158054, 42.513329],
            [20.161942, 42.511665],
            [20.16861, 42.506943],
            [20.220833, 42.433327],
            [20.224163, 42.423332],
            [20.241943, 42.368889],
            [20.241108, 42.364166],
            [20.237778, 42.356667],
            [20.237499, 42.352493],
            [20.24361, 42.33194],
            [20.24472, 42.328606],
            [20.25222, 42.32444],
            [20.2575, 42.323326],
            [20.355675, 42.311844],
            [20.386944, 42.304161],
            [20.450832, 42.278328],
            [20.458054, 42.274437],
            [20.470432, 42.264557],
            [20.508888, 42.23027],
            [20.524166, 42.214439],
            [20.526108, 42.211662],
            [20.595276, 42.04361],
            [20.623333, 41.966942],
            [20.623886, 41.963333],
            [20.621944, 41.954163],
            [20.617775, 41.941666],
            [20.611385, 41.937492],
            [20.595276, 41.931938],
            [20.588608, 41.927498],
            [20.583054, 41.922218],
            [20.581108, 41.918884],
            [20.58083, 41.906662],
            [20.589642, 41.882187],
            [20.568951, 41.874542],
            [20.533607, 41.776939],
            [20.519997, 41.75222],
            [20.516941, 41.743889],
            [20.514164, 41.727776],
            [20.533886, 41.63472],
            [20.46611, 41.55555],
            [20.458054, 41.526382],
            [20.457775, 41.522217],
            [20.458885, 41.518883],
            [20.460552, 41.515831],
            [20.469719, 41.500832],
            [20.471664, 41.497772],
            [20.50861, 41.459717],
            [20.561386, 41.411385],
            [20.562496, 41.408051],
            [20.562222, 41.403885],
            [20.549999, 41.373055],
            [20.524166, 41.350273],
            [20.50222, 41.348053],
            [20.497498, 41.347221],
            [20.496109, 41.34333],
            [20.492775, 41.331108],
            [20.495552, 41.320549],
            [20.502499, 41.304443],
            [20.519165, 41.246384],
            [20.578331, 41.162773],
            [20.600613, 41.093185],
            [20.654999, 41.090271],
            [20.669998, 41.088051],
            [20.733887, 40.911942],
            [20.735119, 40.911057],
            [20.743332, 40.907776],
            [20.75333, 40.905273],
            [20.759163, 40.904442],
            [20.802498, 40.899994],
            [20.811943, 40.901108],
            [20.82111, 40.908882],
            [20.830276, 40.92083],
            [20.840832, 40.932495],
            [20.848331, 40.935555],
            [20.944164, 40.919167],
            [20.957222, 40.914162],
            [20.968887, 40.908607],
            [20.971943, 40.906387],
            [20.982445, 40.895248],
            [20.983887, 40.893608],
            [20.984165, 40.891663],
            [20.98349, 40.855888],
            [20.983887, 40.793053],
            [21.020832, 40.715553],
            [21.028889, 40.708054],
            [21.036587, 40.698563],
            [21.046665, 40.685829],
            [21.051941, 40.676666],
            [21.053055, 40.673332],
            [21.054165, 40.661942],
            [21.053608, 40.622772],
            [21.053055, 40.618607],
            [21.042774, 40.566109],
            [21.041386, 40.561943],
            [20.951942, 40.471382],
            [20.948887, 40.468887],
            [20.945553, 40.466942],
            [20.936943, 40.464439],
            [20.91861, 40.461662],
            [20.914165, 40.463051],
            [20.910831, 40.465271],
            [20.833332, 40.462219],
            [20.791924, 40.431541],
            [20.78722, 40.394722],
            [20.75861, 40.311943],
            [20.740276, 40.309166],
            [20.73333, 40.305275],
            [20.714996, 40.281662],
            [20.713055, 40.278328],
            [20.711941, 40.273888],
            [20.710831, 40.264999],
            [20.714722, 40.255272],
            [20.720833, 40.246666],
            [20.721386, 40.227219],
            [20.721107, 40.223053],
            [20.673885, 40.101387],
            [20.671944, 40.098053],
            [20.665276, 40.094162],
            [20.657219, 40.091385],
            [20.621387, 40.085274],
            [20.604721, 40.075272],
            [20.583332, 40.070549],
            [20.555832, 40.066383],
            [20.48222, 40.064995],
            [20.476944, 40.066109],
            [20.461571, 40.074165],
            [20.449997, 40.072495],
            [20.43861, 40.068054],
            [20.421944, 40.058052],
            [20.316109, 39.993332],
            [20.31472, 39.990273],
            [20.336388, 39.924721],
            [20.338055, 39.921661],
            [20.36972, 39.891388],
            [20.408886, 39.849159],
            [20.41222, 39.84333],
            [20.415276, 39.836937],
            [20.414444, 39.82444],
            [20.41222, 39.815826],
            [20.407219, 39.804443],
            [20.401943, 39.799164],
            [20.385277, 39.788605],
            [20.381386, 39.787216],
            [20.37611, 39.787498],
            [20.371666, 39.788887],
            [20.306129, 39.812119],
            [20.302219, 39.811661],
            [20.29472, 39.808609],
            [20.291386, 39.806664],
            [20.289444, 39.803055],
            [20.298054, 39.771942],
            [20.318054, 39.739998],
            [20.320831, 39.733604],
            [20.319443, 39.729439],
            [20.317497, 39.726105],
            [20.31472, 39.72361],
            [20.221664, 39.648331],
            [20.21833, 39.646385],
            [20.199165, 39.644722],
            [20.193333, 39.645271],
            [20.166664, 39.650276],
            [20.141388, 39.656105],
            [20.105, 39.669441],
            [20.09861, 39.673882],
            [20.093887, 39.679161],
            [20.090553, 39.681389],
            [20.031944, 39.69722],
            [20.021111, 39.697495],
            [20.013332, 39.694717],
            [20.010029, 39.6912],
            [19.994999, 39.686661],
            [19.990555, 39.685829],
            [19.987499, 39.688049],
            [19.982498, 39.696388],
            [19.981941, 39.700272],
            [19.983608, 39.765274],
            [19.987778, 39.867218],
            [19.937496, 39.937492],
            [19.865555, 40.036942],
            [19.863052, 40.039719],
            [19.850277, 40.048607],
            [19.84111, 40.051384],
            [19.825554, 40.054718],
            [19.764999, 40.071938],
            [19.756664, 40.079437],
            [19.749165, 40.090271],
            [19.730553, 40.10305],
            [19.616665, 40.149162],
            [19.510555, 40.193604],
            [19.495552, 40.201385],
            [19.463333, 40.21833],
            [19.452774, 40.224716],
            [19.444721, 40.231941],
            [19.401108, 40.279999],
            [19.288609, 40.417496],
            [19.28833, 40.421104],
            [19.291111, 40.423607],
            [19.318886, 40.438889],
            [19.339443, 40.43222],
            [19.344166, 40.43055],
            [19.386665, 40.393051],
            [19.396664, 40.382774],
            [19.400555, 40.376663],
            [19.405277, 40.363327],
            [19.406109, 40.355827],
            [19.407497, 40.344444],
            [19.415554, 40.32972],
            [19.418888, 40.326942],
            [19.424164, 40.326111],
            [19.433609, 40.325829],
            [19.437878, 40.326431],
            [19.447498, 40.328049],
            [19.457115, 40.332672],
            [19.470833, 40.341942],
            [19.478611, 40.350273],
            [19.480274, 40.359444],
            [19.487499, 40.436661],
            [19.487778, 40.440826],
            [19.45583, 40.561104],
            [19.451385, 40.566666],
            [19.420277, 40.57666],
            [19.414997, 40.577774],
            [19.395832, 40.581108],
            [19.390831, 40.580826],
            [19.337776, 40.618607],
            [19.307209, 40.645309],
            [19.349442, 40.688049],
            [19.35722, 40.696663],
            [19.358887, 40.699997],
            [19.404999, 40.819717],
            [19.41333, 40.866386],
            [19.452419, 40.883842],
            [19.463608, 40.882217],
            [19.46833, 40.88166],
            [19.471664, 40.882774],
            [19.478886, 40.886108],
            [19.521385, 40.908051],
            [19.521942, 40.911659],
            [19.524166, 40.929161],
            [19.523331, 40.936661],
            [19.522778, 40.940277],
            [19.51083, 40.969994],
            [19.506386, 40.975555],
            [19.461109, 41.006943],
            [19.436214, 41.021065]
          ]
        ]
      }
    }
  ]
}
';
    }
}
